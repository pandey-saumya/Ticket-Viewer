...
class ControllerTester(unittest.TestCase):
    ...
    #(1) through menu then quit (q). Then display all (1), go down one page
    # (d) & quit (q)
    ...
    def test_show_all(self, input, test_get):
        ...
        with self.assertRaises(SystemExit) as cm:
            controller.showMainMenu()
        self.assertEqual(cm.exception.code, 0)
        self.assertEqual(controller.currPage, 2)  # I scrolled down one page, so checking if paging happened correctly.
